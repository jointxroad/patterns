...
\subsection{Open questions and TODO}
\begin{itemize}
	\item Is the recommendation for the read-through cache to be non-persistent and short-lived relevant? Also, needs more operational considerations
	\item Is ``shared x-road library`` a pattern? We have seen people state that v6 migration is hard because it touches so many systems but implementing a client lib to encapsulate any protocol is just common sense. Should we include it nevertheless?
\end{itemize}

TODO:
\begin{itemize}
	\item An overview of x-road
\end{itemize}
